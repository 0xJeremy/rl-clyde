...
class ReflexAgent(Agent):
    ...
    def evaluationFunction(self, currentGameState, action):
        ...
        return food_stuff(
            newPos, newFood.asList(), ghost_stuff(newPos, newGhostStates, 2, score)
        )
        ...
        def exp_agent(state, depth, ghost):
            ...
            for action in actions:
                if (
                    ...
                    score = exp_agent(
                        state.generateSuccessor(ghost, action), depth, next_ghost
                    )
                if score < best_score:
                    best_score = score
            return best_score
...
class AlphaBetaAgent(MultiAgentSearchAgent):
    ...
    def getAction(self, gameState):
        ...
        def max_agent(state, depth, alpha, beta):
            ...
            for action in actions:
                score = min_agent(
                    state.generateSuccessor(PACMAN, action), depth, 1, alpha, beta
                )
                if score > best_score:
                    best_score = score
                    best_action = action
...
def betterEvaluationFunction(currentGameState):
    ...
    score = (
        score * 2
        if closest_dot(pacman_pos, food) < closest_ghost(pacman_pos, ghosts) + 3
        else score
    )
